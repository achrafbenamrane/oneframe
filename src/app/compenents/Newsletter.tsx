...
const Newsletter = () => {
  ...
  return (
    <div className="relative w-full py-20 flex items-center justify-center overflow-hidden p-4 bg-white/80 dark:bg-[url(/emailbg.jpg)] bg-cover bg-center dark:bg-cover dark:bg-center backdrop-blur-sm transition-colors duration-300">
      <motion.div
      ...
      >
        ...
        <motion.form
          ...
          variants={itemVariants}
        >
          <div className="relative flex flex-col sm:flex-row items-center bg-white dark:bg-gray-800 p-2 rounded-full shadow-lg border border-gray-200 dark:border-gray-700 group hover:border-cyan-500 dark:hover:border-cyan-400 focus-within:ring-2 focus-within:ring-cyan-500 focus-within:ring-offset-2 dark:focus-within:ring-offset-gray-900 transition-all duration-300">
            <Phone className={`absolute ${lang === 'ar' ? 'right-5' : 'left-5'} top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400 dark:text-gray-500 hidden sm:block`} />
            ...
            <div className="w-full">
              <input
                ...
                className={`w-full sm:w-auto flex-grow bg-transparent ${
                  lang === 'ar' ? 'sm:pr-12 text-right' : 'sm:pl-12 text-left'
                } px-4 py-3 text-gray-700 dark:text-gray-200 placeholder:text-gray-500 dark:placeholder:text-gray-400 outline-none text-center sm:text-left ${
              ...
              />
              {phoneError && (
                <p className={`text-sm text-red-500 mt-1 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
                  {phoneError}
                </p>
              )}
              {submitStatus === 'success' && (
                <p className={`text-sm text-green-500 mt-1 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
            ...
            <button
              ...
              className={`w-full sm:w-auto mt-2 sm:mt-0 px-6 py-3 bg-cyan-600 dark:bg-cyan-500 text-white font-semibold rounded-full transition-colors duration-300 shadow-md transform ${
                isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:bg-cyan-700 dark:hover:bg-cyan-600 group-hover:scale-105'
              }`}
            >
              {isSubmitting ? (
        ...
        >
          ...
          <p className="text-gray-600 dark:text-gray-400">
            <span className="font-semibold text-gray-800 dark:text-gray-200">10k+ </span>
            {translations.joinedText[lang as keyof typeof translations.joinedText]}
          </p>
        </motion.div>
      </motion.div>
    </div>
